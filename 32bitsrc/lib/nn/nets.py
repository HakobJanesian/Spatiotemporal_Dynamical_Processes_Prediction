...
class FCN(nn.Module):
    def __init__(self, layers_list, activation_function_list = None, device = "cpu"):
        ...
        for i, activation_function in enumerate(activation_function_list):
            seq.append(('layer_%d' % i, torch.nn.Linear(layers_list[i], layers_list[i+1], dtype = torch.float32)))
            seq.append(('activation_%d' % i, activation_function()))
        
        seq.append(('layer_%d' % (self._depth - 1), torch.nn.Linear(layers_list[-2], layers_list[-1], dtype = torch.float32)))
...
class MShuffle(nn.Module):
    def __init__(self, exp_size, n_depth = None, again = 1, activation_function_list = None, device = "cpu"):
        nn.Module.__init__(self)
        if n_depth is None:
            n_depth = exp_size
        ...
        for ag in range(again):
            for n, activation_function in enumerate(activation_function_list):
                seq.append(('layer_ag_%d_n_%d' % (ag + 1, n + 1), torch.nn.Linear(2**exp_size, 2**(exp_size-1), dtype = torch.float32)))
